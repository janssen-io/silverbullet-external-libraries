...
// ../../../Data/Software/silverbullet-github-libraries-plug/hello.ts
async function helloWorld() {
  const allExternalLibraries = await system_exports.getSpaceConfig("externalLibraries", []);
  const fileListCache = {};
  for (let lib of allExternalLibraries) {
    ...
    if (scheme === "gh") {
      ...
      if (pathParts[0] !== "Library") {
        await editor_exports.flashNotification("Skipping library " + lib);
      ...
      if (fileListCache[cacheKey] === void 0) {
      ...
      const libraryFiles = fileListCache[cacheKey].filter((entry) => entry.name.startsWith(path));
